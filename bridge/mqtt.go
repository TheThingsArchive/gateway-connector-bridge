...
func (b *Bridge) SetupMQTT(config MQTTConfig) error {
	mqtt := new(MQTT)

	mqtt.ctx = b.Ctx.WithField("Connector", "MQTT")
...
// MQTTConfig contains configuration for MQTT
...
func (c *MQTT) SubscribeConnect() (<-chan *ConnectMessage, error) {
	messages := make(chan *ConnectMessage, BufferSize)
...
func (c *MQTT) SubscribeDisconnect() (<-chan *DisconnectMessage, error) {
	messages := make(chan *DisconnectMessage, BufferSize)
...
func (c *MQTT) SubscribeUplink(gatewayID string) (<-chan *UplinkMessage, error) {
	ctx := c.ctx.WithField("GatewayID", gatewayID)
	messages := make(chan *UplinkMessage, BufferSize)
	token := c.subscribe(fmt.Sprintf(UplinkTopicFormat, gatewayID), func(_ paho.Client, msg paho.Message) {
		...
		select {
		case messages <- &UplinkMessage{gatewayID: gatewayID, message: &uplink}:
...
func (c *MQTT) SubscribeStatus(gatewayID string) (<-chan *StatusMessage, error) {
	ctx := c.ctx.WithField("GatewayID", gatewayID)
	messages := make(chan *StatusMessage, BufferSize)
	token := c.subscribe(fmt.Sprintf(StatusTopicFormat, gatewayID), func(_ paho.Client, msg paho.Message) {
		...
		select {
		case messages <- &StatusMessage{gatewayID: gatewayID, message: &status}:
...
func (c *MQTT) PublishDownlink(message *DownlinkMessage) error {
